    ...
    public class GizmosInstance : MonoBehaviour
    {
        ...
        internal static GizmosInstance GetOrCreate()
        {
            if (hotReloaded || !instance)
            {
                ...
                if (!instance)
                {
                    instance = new GameObject("Gizmos Instance").AddComponent<GizmosInstance>();
                    //instance.gameObject.hideFlags = HideFlags.HideInHierarchy | HideFlags.HideInInspector; // Don't hide it gizmo object, it should always be in the scene

                    markDirty = true;
                }

                // Don't hide it gizmo object, it should always be in the scene
        ...
        internal static void Submit(Vector3[] points, Color? color, bool dashed)
        {
            ...
            if (inst.lastFrame != frameCount)
            {
                inst.lastFrame = frameCount;
                inst.queueIndex = 0;
            }
